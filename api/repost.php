...
$check = $pdo->prepare('SELECT id FROM reposts WHERE post_id = :post_id AND user_id = :user_id');
$check->execute(['post_id' => $postId, 'user_id' => $user['id']]);
$existing = $check->fetchColumn();

if ($existing) {
    $del = $pdo->prepare('DELETE FROM reposts WHERE id = :id');
    $del->execute(['id' => $existing]);
    $reposted = false;
} else {
    $ins = $pdo->prepare('INSERT INTO reposts (post_id, user_id) VALUES (:post_id, :user_id)');
    $ins->execute(['post_id' => $postId, 'user_id' => $user['id']]);
    $reposted = true;
}

$stats = post_stats($postId, (int) $user['id']);
json_response(['ok' => true, 'reposted' => $reposted, 'reposts' => $stats['reposts']]);
